...
async function preloadMarkerIcons(){ const urls=Object.keys(markerColors).map(color=>new URL(`./assets/markers/pin-${color}.svg`,location.href).href); try { await Promise.all(urls.map(url=>new Promise((resolve,reject)=>{const image=new Image();image.onload=resolve;image.onerror=reject;image.src=url;}))); return true; } catch { return false; } }
...
function markerIcon(color){ if(!state.markerIconsReady)return undefined; const AMap=state.amap, size=new AMap.Size(32,40); return new AMap.Icon({size,image:new URL(`./assets/markers/pin-${color}.svg`,location.href).href,imageSize:size}); }
function createPointMarker(group){ const count=group.records.length, color=count>1?'multi':markerColorFor(group.records[0]), marker=new state.amap.Marker({position:group.position,icon:markerIcon(color),offset:new state.amap.Pixel(-16,-40),anchor:'bottom-center',zIndex:120,extData:{key:group.key},title:count>1?`${count} 家餐厅`:group.records[0].name}); if(count>1)marker.setLabel({content:String(count),direction:'center',offset:new state.amap.Pixel(0,-25)}); marker.on('click',()=>count===1?openDetail(group.records[0].id):openCoordinateGroup(group)); return marker; }
...
function renderPointMarkers(renderId){ if(!state.map||!state.amap||!state.mapReady||state.layersHidden||renderId!==state.markerRenderId)return; const groups=selectedGroupsInView(), next=new Map(groups.map(group=>[group.key,{group,signature:pointSignature(group)+(group.records.length===1?':'+markerColorFor(group.records[0]):'')} ])); for(const [key,current] of state.visiblePointMarkers){ const desired=next.get(key); if(!desired||current.signature!==desired.signature){state.map.remove(current.marker);state.visiblePointMarkers.delete(key);state.pointLayerRebuilds++;} } for(const [key,desired] of next){if(!state.visiblePointMarkers.has(key)){const marker=createPointMarker(desired.group);state.map.add(marker);state.visiblePointMarkers.set(key,{...desired,marker});}} state.pointLayerUpdates++; window.__FOOD_MAP_DIAGNOSTICS={mapInitCount:state.mapInitCount,filtered:state.filtered.length,validCoordinates:state.records.filter(hasCoords).length,uniqueCoordinates:state.coordinateGroups.size,renderedPoints:state.visiblePointMarkers.size,markerUpdates:state.pointLayerUpdates,markerRebuilds:state.pointLayerRebuilds,markerType:'AMap.Marker',markerIconsReady:state.markerIconsReady,restaurantLayersHidden:state.layersHidden,visibleRestaurantPoints:state.layersHidden?0:state.visiblePointMarkers.size}; if(!state.initialFitDone&&state.visiblePointMarkers.size)state.initialFitDone=true; }
